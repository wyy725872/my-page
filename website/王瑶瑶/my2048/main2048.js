    ...
    for(var j=0;j<4;j++){
         $("#grid-container").append(' <div class="number-cell" id="number-cell-'+i+'-'+j+'"></div>');
             ...
             if(board[i][j]==0){
                 ...
                 theNumberCell.css('top',getPosTop(i,j)+50);//数字在格子中间显示
...
function moveLeft(){
    ...
    for(var j=1;j<4;j++){
        if( board[i][j]!=0){
            for( var k=0;k<j;k++){
                if(board[i][k]==0&& noBlockHorizontal(i,k,j,board)){//判断能否想做移动条件一  左侧有空
                    showMoveAnimation(i,j,i,k);
                    board[i][k]=board[i][j];
                    board[i][j]=0;
                    continue;
                }
                else if(board[i][k]==board[i][j]&&noBlockHorizontal(i,k,j, board)&&!hasConflicted[i][k]){//二左侧数字和本身相等
                    showMoveAnimation(i,j,i,k);
                    board[i][k]+=board[i][j];
                    board[i][j]=0;
                    score+=board[i][k];
